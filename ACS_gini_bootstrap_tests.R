...
result_temp<-ddply(Gini_try, .variables=c("MSA"), function(x) c("P_val"=Gini_diff_boot(x, 100, 100, 2005, 2011)), .parallel=T)
#result_temp<-ddply(Gini_try, .variables=c("State"), function(x) c("P_val"=Gini_diff_boot(x, 100, 100, 2005, 2011)), .parallel=T)

ACS.work.hh<-count<-1
attempt1<-aggregate(count~MSA, data=ACS.work.hh, FUN==sum)