...
perc_boot_virt<-function(data, virtname, ord, reps=500){
  bootreps<-ddply(data, .variables=c("variable"), function(x) data.frame("replicates"=perc_replicates(data, ord, reps), 
                                                                       ...
                                                                       Lorenz_KB(data[which(data$name==1),]$value, rep(1, length(data[which(data$name==2),]$value)), ord, type="mean"), reps)),.parallel=T)
...
virt_inc1<-matrix(rep(0, 2000*11), 2000, 11)
virt_inc2<-matrix(rep(0, 2000*11), 2000, 11)
for (i in 1:10){
...
virt_inc1[,11]<-rep(1, 2000)
virt_inc2[,11]<-rep(2, 2000)
...
save(boot_results, file="perc_LC_boot_GB2.rda")
